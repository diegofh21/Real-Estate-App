...
import { getProductosFacturaPDF, RequestPage, GetClienteRIF } from '../../api/request';
...
export const FacturaPDF = (props) => {
  ...
  Fecha = Factura.FechaDocumento.slice(0, 10)

  // if (props.FacturaBuscada) {
  //   console.log(Factura)
  //   CodigoCliente = Factura.Codcliente
  //   Documento = Factura.Documento
  //   Fecha = Factura.fechayhora.split(" ")[0]
  // }
  ...
  const generatePDF = async () => {
    ...
    doc.text("Factura", 175, 40, null, null, "center");
    ...
    doc.text("Páginas", 187, 69, null, null, "center");
    ...
    const productos = await getProductosFacturaPDF(Documento);
    ...
    for (let i = 0; i < productos.data.length; i++) {
      ...
      doc.text("$" + productos.data[i].PrecioUnitario.toFixed(2), 145, textSpacing)
    ...
    doc.text("Descuento: $"+Factura.Descuento, 15, 270)
    doc.text("Total neto: $"+Factura.TotalFinal, 140, 270)
    //FIN PIE DE PÁGINA
